...
public class PaymentServiceImpl implements PaymentService {
	...
	public PaymentBean savePayment(PaymentBean paymentBean,Orders orderEntity) {
	    ...
	    if (paymentBean.getAmount() <= 0 && orderEntity.getOrderId() <= 0 && paymentBean.getPaymentMode().isEmpty() ) {
	    	...
	    	beanToEntity(payment, paymentBean);
	...
	public void beanToEntity(Payment payment, PaymentBean paymentBean) {
		payment.setAmount(paymentBean.getAmount());
		payment.setPaymentDate(LocalDateTime.now());
		payment.setPaymentMode(paymentBean.getPaymentMode());
		payment.setStatus(paymentBean.getStatus());
	}
